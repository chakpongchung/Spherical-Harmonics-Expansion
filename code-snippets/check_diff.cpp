...
int main() {
  ...
  double theta = M_PI / 3, phi = M_PI / 4; // theta θ in [0, π], phi φ in
                                           // [0,2π);
  ...
  for (int l = 0; l <= num; l++) {
    ...
    if (!(diff < truncation_error) && num < 50) // continuing conditions: if
                                                // truncation error not met and
                                                // index l is not over limit
      num++;
  }
