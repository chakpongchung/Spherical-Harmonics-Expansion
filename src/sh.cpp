...
double sh::a(int l_prime, int m_prime, int l1, int m1, int l2, int m2)
{
    ...
    // printf("temp is: %f\n", temp);
    double first =
        gsl_sf_coupling_3j(2 * l1, 2 * l2, 2 * l_prime, 2 * 0, 2 * 0, 2 * 0);
    // printf("first is: %f\n", first);
    double second = gsl_sf_coupling_3j(2 * l1, 2 * l2, 2 * l_prime, 2 * m1,
                                       2 * m2, -2 * m_prime);
    // printf("second is: %f\n", second);
...
double sh::b(int m_prime, int l_prime, int m1, int l1, int m2, int l2, int m3,
             int l3, int num /*= 16*/)
{
    double sum = 0;
    for (int l = 0; l <= num; l++)
...
void sh::print_a()
{
    printf(
        "*******************************************************************\n");
    printf(
        "cofficients a(n,m+p,j,p,l,m)                                       \n");
    printf(
        "for spherical-harmonic expansion test program  -cpchung            \n");
    printf(
...
void sh::check_diff()
{
    printf(
        "*******************************************************************\n");
    printf(
        "check_diff() between lfs and rhs                                   \n");
    printf(
        "for spherical-harmonic expansion  -cpchung                         \n");
    printf(
        "data saved to data.csv                                             \n");
    printf(
        "*******************************************************************\n");
    ...
    for (int l = 0; l <= num; l++)
    {
        for (int m = -l; m <= l; m++)
            ...
            {
                printf("a_lm, l, m,  l1, m1 , l2, m2 are: %.15f %2d %2d %2d %2d %2d "
                       "%2d  \n\n",
                       a(l, m, l1, m1, l2, m2), l, m, l1, m1, l2, m2);

                sum = sum +
                      a(l, m, l1, m1, l2, m2) * spherical_harmonic(l, m, theta, phi);

                datafile << l << "," << m << "," << l1 << "," << m1 << "," << l2 << ","
                         << m2 << "," << a(l, m, l1, m1, l2, m2) << endl;
            }

        diff = abs(sum -
                   spherical_harmonic(l1, m1, theta, phi) *
                       spherical_harmonic(l2, m2, theta, phi));

        // continuing conditions: if truncation error not met and index l is not
        // over limit
